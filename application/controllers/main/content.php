...
class content extends MY_Controller  {
    ...
    */
    ...
    public function index($p = 0)
    {
        ...
        //1是特殊的需要看详情的
        $flag = in_array($this->type , parent :: $detail_data) ? 1 : 0;
   ...
   */
    public function member($user_sn){
        ...
        $group_count = $this->model_content->group_count_by_type($where);
        foreach($group_count as &$value){
            $count += $value['num'];
            $value['type_name'] = parent :: $all_type_name[$value['type']];
        }
        array_push($group_count , array('type'=>0,'type_name'=>'全部','num'=>$count));
    ...
    */
    public function member_list(){
        ...
        foreach($list as &$v){
            $v['create_time'] = change_time($v['create_time']);
            //1是特殊的需要看详情的
            $v['is_detail'] = in_array($v['type'], parent :: $detail_data) ? 1 : 0;
            $v['u_name'] = empty($v['user_id']) ? md5($v['email']) : $v['name'];
            if($v['is_detail'] == 1){
                $v['content'] = mb_substr(strip_tags($v['content']), 0, 150, 'utf-8').'...';
                $v['title'] = empty($v['title']) ? mb_substr($v['content'], 0, 20, 'utf-8').'...' : $v['title'];
